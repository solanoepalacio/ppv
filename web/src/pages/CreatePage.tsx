...
import CreateChecklist, {
	type ChecklistStep,
	type StepStatus,
} from "../components/CreateChecklist";
...
export default function CreatePage() {
	...
	const pricePlanck = priceInput ? BigInt(Math.round(parseFloat(priceInput) * 1e12)) : 0n;

	const section: Section = !videoFile
		? "A"
		: !thumbnailBytes
			? "B"
			: title.length < 1 ||
	...
	function handleFilePick(file: File) {
		...
		if (file.size > MAX_UPLOAD_BYTES - 40) {
			setVideoError(
				"File is too large. Phase-2 PoC supports videos up to ~2 MiB of plaintext (encryption adds 40 bytes).",
			);
			return;
		}
	...
	function setStep(id: string, status: StepStatus, detail?: string, errorMsg?: string) {
		setSteps((prev) => prev.map((s) => (s.id === id ? { ...s, status, detail, errorMsg } : s)));
	}
	...
	async function handleSubmit() {
		...
		try {
			...
			const thumbnailCid = await uploadToBulletin(thumbnailBytes, (pct) =>
				setStep("thumb", "running", `${Math.round(pct)}%`),
			);
			setStep("thumb", "done");

			setStep("content", "running");
			const contentCid = await uploadToBulletin(ciphertextBytes, (pct) =>
				setStep("content", "running", `${Math.round(pct)}%`),
			);
	...
	const submitDisabled = section !== "D" || submitting || !encryptionKey.ready;
	const submitLabel = encryptionKey.ready ? "Create listing" : "Preparing encryption key…";
	...
	return (
		<div className="max-w-5xl">
			...
			<div className="flex flex-col lg:flex-row gap-8">
				{/* Left column: video picker + thumbnail strip */}
				<div className="lg:flex-1 flex flex-col gap-4">
					...
					{videoError && <p className="text-accent-red text-xs">{videoError}</p>}

					{videoFile ? (
						<ThumbnailPicker
							videoFile={videoFile}
							onSelect={(bytes) => setThumbnailBytes(bytes)}
						/>
					) : (
						<div className="flex flex-col gap-2">
							...
							<div className="flex gap-2">
								{[0, 1, 2].map((i) => (
									<div
										key={i}
										className="flex-1 aspect-video rounded-lg border border-white/10 bg-surface-900/40"
									/>
								))}
							</div>
						</div>
				...
				{/* Right column: metadata (gated on video + thumbnail) */}
				<div className="lg:flex-1 flex flex-col gap-4">
			...
			<div className="mt-8 flex justify-center">
				{steps.length === 0 ? (
					<button
						...
						className="px-8 py-2.5 rounded-lg bg-polka-500 hover:bg-polka-400 text-white text-sm
					...
					<div className="w-full max-w-xl">
						<CreateChecklist steps={steps} onRetry={() => handleSubmit()} />
					</div>
				)}
			</div>
		</div>
