...
let ppviewDescriptorPromise: Promise<
	(typeof import("@polkadot-api/descriptors"))["ppview"]
> | null = null;

let connectId = 0;

async function getPpviewDescriptor() {
	if (!ppviewDescriptorPromise) {
		ppviewDescriptorPromise = import("@polkadot-api/descriptors").then(
			({ ppview }) => ppview,
		);
	}

	return ppviewDescriptorPromise;
}
	...
	const connect = useCallback(
		async (url: string) => {
			...
			try {
				...
				const descriptor = await getPpviewDescriptor();
